  ...
  xml.channel do
    ...
    @rss_urls.each do |r|
      xml.item do
        ...
        xml.link projects_slug_path(Project.find(@rss_feed.subscription.project_id).slug)
        xml.guid projects_slug_path(Project.find(@rss_feed.subscription.project_id).slug)
      end
    end
  end
end
